...
class RewardService {
  ...
  static const String _storyModeCompletedKey = 'story_mode_completed';
  ...
  /// 스토리 모드 완료 여부 가져오기
  static Future<bool> isStoryModeCompleted() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getBool(_storyModeCompletedKey) ?? false;
  }

  /// 스토리 모드 완료 처리 (별 보상 지급)
  static Future<void> completeStoryMode() async {
    final prefs = await SharedPreferences.getInstance();

    // 이미 완료했다면 중복 지급 방지
    final alreadyCompleted = prefs.getBool(_storyModeCompletedKey) ?? false;
    if (alreadyCompleted) return;

    // 완료 플래그 저장
    await prefs.setBool(_storyModeCompletedKey, true);

    // 스토리 클리어 보상으로 별 100개 지급
    final currentStars = await getTotalStars();
    await prefs.setInt(_totalStarsKey, currentStars + 100);
  }
}
